...
const Divider = () => <div className="w-full h-px bg-black-500 mt-8 mb-4" />;
...
const SideBar = ({ isOpen, setIsOpen }: SideBarProps) => {
  ...
  return (
    <>
      {isOpen && (
        <div className="w-screen h-screen bg-[#141414]/60 fixed bottom-11 left-0 right-0 top-0 pc:hidden z-0" />
      )}

      <nav
        ref={navigationRef}
        className={cn(
          "fixed top-0 bottom-11 right-0 bg-white w-66.75 z-50",
          "transform duration-150 transition-transform ease-in-out",
          isOpen ? "translate-x-0" : "translate-x-full",
          "px-6 py-8 flex flex-col justify-between pc:hidden",
        )}
      >
          ...
          <div className="flex flex-col">
            {NAV_LINKS.map(({ href, label }) => (
              <Link
                key={href}
                href={href}
                className="text-start px-1 py-2 typo-mo-body-l400 hover:bg-black-200 active:bg-black-200"
              >
                {label}
              </Link>
            ))}
        ...
        <div className="flex flex-col">
              ...
              {COLOR_BG_CLASS.map(({ color, bgClass }) => {
                return (
                  <button
                    ...
                    onClick={() => setCurrentColor(color as BackgroundColor)}
                  />
                );
              })}
            </div>
          ...
          <button
            ...
            className="flex items-center justify-between px-1 py-2 typo-mo-body-l400 hover:bg-black-200 active:bg-black-200 cursor-pointer"
          >
            로그아웃
